...
public class ParticipantService:IParticipantService
{
    ...
    public async Task<IEnumerable<ParticipantDTO>> GetByEventIdAsync(Guid eventId, CancellationToken cancellationToken=default)
    {
        ...
        return _mapper.Map<IEnumerable<ParticipantDTO>>(participantsByEvent);
    }

    public async Task<IEnumerable<ParticipantDTO>> GetByUserId(Guid userId, CancellationToken cancellationToken=default)
    {
        var user = await _unitOfWork.Users.GetByIdAsync(userId, cancellationToken) 
                   ?? throw new EntityNotFoundException("User",userId);
        var participantsByUser = await _unitOfWork.Participants.GetByUserId(userId, cancellationToken);
        return _mapper.Map<IEnumerable<ParticipantDTO>>(participantsByUser);
    }
}